...
def Search():
    ...
    column1 = [
        [sg.Text('Busca Usuário', size=(14,0)),sg.Input(size=(30,0),key='busca'),sg.Button('Buscar',size=(6,0),bind_return_key=True)],
    ...
    searchequipmentuser = sg.Window('S.G.T.I. - Busca Equipamento Por Usuário', layout, icon='Images/icon.ico', finalize=True)
    ...
    while True:
        ...
        if evento == 'Buscar':
            if valores['busca'] == '':
                sg.popup_auto_close('Digite algum usuário', auto_close_duration=2, no_titlebar=True, button_type=5, background_color='#8c8c8c', text_color='#ededed', font=('Arial Black',12))
                sg.cprint('')
            else:
                try:
                    DataBase.cursor.execute("SELECT user FROM equipments WHERE user LIKE '%{}%'".format(valores['busca']))
                    resultado = DataBase.cursor.fetchall()
                    sg.cprint("Usuário: ",resultado[0][0])
                    searchequipmentuser['user'].update(resultado[0][0])
                    DataBase.cursor.execute("SELECT place FROM equipments WHERE user LIKE '%{}%'".format(valores['busca']))
                    resultado = DataBase.cursor.fetchall()
                    searchequipmentuser['place'].update(resultado[0][0])
                    sg.cprint("Local: ",resultado[0][0])
                    DataBase.cursor.execute("SELECT department FROM equipments WHERE user LIKE '%{}%'".format(valores['busca']))
                    resultado = DataBase.cursor.fetchall()
                    searchequipmentuser['department'].update(resultado[0][0])
                    sg.cprint("Departamento: ",resultado[0][0])
                    DataBase.cursor.execute("SELECT office_license FROM equipments WHERE user LIKE '%{}%'".format(valores['busca']))
                    resultado = DataBase.cursor.fetchall()
                    searchequipmentuser['office_license'].update(resultado[0][0])
                    sg.cprint("Office: ",resultado[0][0])
                    DataBase.cursor.execute("SELECT office_type FROM equipments WHERE user LIKE '%{}%'".format(valores['busca']))
                    resultado = DataBase.cursor.fetchall()
                    searchequipmentuser['office_type'].update(resultado[0][0])
                    sg.cprint("Office Tipo: ",resultado[0][0])
                    DataBase.cursor.execute("SELECT office_key FROM equipments WHERE user LIKE '%{}%'".format(valores['busca']))
                    resultado = DataBase.cursor.fetchall()
                    searchequipmentuser['office_key'].update(resultado[0][0])
                    sg.cprint("Office Chave: ",resultado[0][0])
                    DataBase.cursor.execute("SELECT windows_license FROM equipments WHERE user LIKE '%{}%'".format(valores['busca']))
                    resultado = DataBase.cursor.fetchall()
                    searchequipmentuser['windows_license'].update(resultado[0][0])
                    sg.cprint("Windows: ",resultado[0][0])
                    DataBase.cursor.execute("SELECT windows_type FROM equipments WHERE user LIKE '%{}%'".format(valores['busca']))
                    resultado = DataBase.cursor.fetchall()
                    searchequipmentuser['windows_type'].update(resultado[0][0])
                    sg.cprint("Windows Tipo: ",resultado[0][0])
                    DataBase.cursor.execute("SELECT windows_key FROM equipments WHERE user LIKE '%{}%'".format(valores['busca']))
                    resultado = DataBase.cursor.fetchall()
                    searchequipmentuser['windows_key'].update(resultado[0][0])
                    sg.cprint("Windows Chave: ",resultado[0][0])
                    DataBase.cursor.execute("SELECT equipment FROM equipments WHERE user LIKE '%{}%'".format(valores['busca']))
                    resultado = DataBase.cursor.fetchall()
                    searchequipmentuser['equipment'].update(resultado[0][0])
                    sg.cprint("Equipamento: ",resultado[0][0])
                    DataBase.cursor.execute("SELECT manufacturer FROM equipments WHERE user LIKE '%{}%'".format(valores['busca']))
                    resultado = DataBase.cursor.fetchall()
                    searchequipmentuser['manufacturer'].update(resultado[0][0])
                    sg.cprint("Marca: ",resultado[0][0])
                    DataBase.cursor.execute("SELECT id_equipment FROM equipments WHERE user LIKE '%{}%'".format(valores['busca']))
                    resultado = DataBase.cursor.fetchall()
                    searchequipmentuser['id_equipment'].update(resultado[0][0])
                    sg.cprint("TAG: ",resultado[0][0])
                    DataBase.cursor.execute("SELECT provider FROM equipments WHERE user LIKE '%{}%'".format(valores['busca']))
                    resultado = DataBase.cursor.fetchall()
                    searchequipmentuser['provider'].update(resultado[0][0])
                    sg.cprint("Fornecedor: ",resultado[0][0])
                    DataBase.cursor.execute("SELECT invoice FROM equipments WHERE user LIKE '%{}%'".format(valores['busca']))
                    resultado = DataBase.cursor.fetchall()
                    searchequipmentuser['invoice'].update(resultado[0][0])
                    sg.cprint("Nota Fiscal: ",resultado[0][0])
                    DataBase.cursor.execute("SELECT access_key FROM equipments WHERE user LIKE '%{}%'".format(valores['busca']))
                    resultado = DataBase.cursor.fetchall()
                    searchequipmentuser['access_key'].update(resultado[0][0])
                    sg.cprint("Chave Acesso: ",resultado[0][0])
                    searchequipmentuser['busca'].update('')
                except:
                    sg.popup_auto_close('Usuário não localizado', auto_close_duration=2, no_titlebar=True, button_type=5, background_color='#8c8c8c', text_color='#ededed', font=('Arial Black',12))
